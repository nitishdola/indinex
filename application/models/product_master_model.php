...
class Product_master_model extends CI_Model 
{
  ...
  public function select_pos_items()  
  {  
    $this->db->select('*');
    $this->db->select('product_general_data.id as product_general_data_id');
    $this->db->from('product_general_data');
    ...
    $this->db->where('product_purchase_data.items','sale_item');
    $this->db->order_by("product_general_data.id", "desc");
    $query = $this->db->get();  
    return $query; 
  ...
  public function change_product_general_data($product_code,$product_category,$product_description,$product_group,$picture,$old_material_no,$size,$color,$conversion_factor_from,$factor_from_uom,$conversion_factor_to,$factor_to_uom){
    $query=$this->db->query("update product_general_data SET product_category='$product_category',product_description='$product_description',product_group='$product_group',picture='$picture',old_material_no='$old_material_no',size='$size',color='$color',conversion_factor_from='$conversion_factor_from',factor_from_uom='$factor_from_uom',conversion_factor_to='$conversion_factor_to',factor_to_uom='$factor_to_uom' where product_code='$product_code'");
    return true;
  }
  public function change_product_purchase_data($product_code,$plant,$storage_location,$packaging,$packaging_uom,$order_unit_uom,$order_unit,$shipping_instructions,$max_tolerance,$min_tolerance,$min_order_qty,$min_order_qty_uom,$manufacture_part_no,$manufacturer_name,$items){
    $query=$this->db->query("update product_purchase_data SET plant='$plant',storage_location='$storage_location',packaging='$packaging',packaging_uom='$packaging_uom',order_unit_uom='$order_unit_uom',shipping_instructions='$shipping_instructions',max_tolerance='$max_tolerance',min_tolerance='$min_tolerance',min_order_qty='$min_order_qty',min_order_qty_uom='$min_order_qty_uom',manufacture_part_no='$manufacture_part_no',manufacturer_name='$manufacturer_name',items='$items' where product_code='$product_code'");
    return true;
  }

  public function change_product_manufacturing_data($product_code,$product_manufacturing,$manufacturing_date,$product_purchase,$product_make_to_order,$in_house_production,$ok_to_purchase){
    $query=$this->db->query("update  product_manufacture_data SET product_manufacturing='$product_manufacturing',manufacturing_date='$manufacturing_date',product_purchase='$product_purchase',product_make_to_order='$product_make_to_order',in_house_production='$in_house_production',ok_to_purchase='$ok_to_purchase' where product_code='$product_code'");
    return true;
  }
  ...
  public function change_product_accounting_data($product_code,$ledger,$currency,$sale_price,$purchase_price){
    $query=$this->db->query("update  product_accounting_data SET product_code='$product_code',ledger='$ledger',currency='$currency',sale_price='$sale_price',purchase_price='$purchase_price' where product_code='$product_code'");
    return true;
  }
